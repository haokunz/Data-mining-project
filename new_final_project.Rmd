...
\flushleft
After exploring the data set, we continued to cluster with the companies based on the following values: audit fees($), total fees($), Market capitalization($), market-fee ratio, asset value($) after natural log transformation and transformed revenue and earnings. We used the Gap Statistic and K-means methods to determine the number of clusters.
...
# K-means Clustering
The K-means clustering uses a principal component analysis to create clusters, classify and partition objects into multiple groups that the objects are as similar as possible within, and as dissimilar to the objects in other groups as possible. K represents the number of groups. From the previous result, we select 2 sets of groups, with the same variables as Gap statistic used.  
```{r fig9, fig.align='center', fig.cap="K-means Clustering Result" warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
knitr::kable(clusters,
             caption = "Characteristics of K-means Clustering Groups",
             digits = 3)
...
```
The K-means clustering aggregated 2 groups that have similar features. The Figure \ref@(fig:fig9) shows that both groups overlap each other in many areas. The component of the first principal component accounts for 55.8% of the total variation. And the second principal component accounts for 19.4% Together they explain 75.2% variation in the data set.
