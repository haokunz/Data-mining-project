...
# We can see that linear model has the lowest MAE, rf is no.2
# GBM has the lowest gbm, rf is no.2
...
```
From the above figures, we. can see that linear model has the lowest MAE, rf is no.2, 
GBM has the lowest gbm, rf is no.2
So we decided to choose these 3 models initially.
Then we do the advanced feature engineering.
...
```
Depend on the above figures, we saw that random forest model has the best performance, which has the lowest MAE, lowest RMSE and highest R-square. 
...
```
????????The plot of rf1 shows out-of-bag MSE as a function of the number of trees used, We saw that when the number of tree equal to about 200, the MSE is slowing down. So we choose number of tree = 200.???????要選 number of trees = 200 嗎？
