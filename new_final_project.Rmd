...
```{r fig1, fig.cap="Effect of normalization on variables related to fee", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```{r fig2, fig.cap="Distribution of numbers of companies in diffeerent regions", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```
\flushleft
Figure 2 shows geographical distribution of public companies on the U.S. stock exchanges. US West region has the highest number of public companies, while Canada has the least number of pubic companies on the U.S. stock exchanges.

```{r fig3, fig.cap="Distribution of Categorical Variables", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```
\flushleft
From the above graph, we see that the distribution of market cap of companies whose auditors are non-big 4 firms is obviously lower than whose auditors are big 4. In addition, we also find that companies whose auditors are non-big 4 firms have higher tendency of ineffective internal control. Companies having higher market cap show higher numbers of effective internal controls. 

```{r fig5, fig.cap="Relationship between Total Fee and Market Capitalization", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
market_fee_correlation <- cor(df3$market_cap_bc, df3$total_fees_bc)
knitr::kable(market_fee_correlation,
             "simple",
             caption = "Correlation of Market Cap and Total Fee",
             digits = 3) %>%
  kable_styling(latex_options = c("scale_down"))

```
\flushleft
From the Figure 5, we can see that the non-Big 4 firms are obviously on the lower left corner. This represents that market cap of the companies which are audited by the non-Big 4 is lower, and the total fee the company pay for audit is also lower. That's reasonable because the Big 4 has their brand value.

```{r fig6, fig.cap="Total fees of Different Auditor Types", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```
\flushleft
Figure 6 shows that generally all Big 4 firms have higher median audit fees than non-Big 4 firms. We can see that the highest audit fee is charged by one Big 4 firm, Deloitte and the lowest audit fee is also charged by another Big 4 firm PricewaterhouseCoopers LLP instead of non-Big 4 firms. Whether the auditor is one of the Big 4 firms isn't the only factor determining audit fees. Therefore, what are the key factors to determine audit fees need predictive model to explore.

# Perform a statistical test here to compare big4 vs non big4 when considering
```{r fig7, fig.cap="Average Audit Fee Comparison Between Audit Types", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
# perform a statistical test here to compare big4 vs non big4 when considering
...
```{r fig8, fig.cap="Gap Statistic Clustering Result", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE, results= FALSE, fig.keep='all'}
...
```
\flushleft
As The Figure 8 suggests, the Gap statistic estimates that the optimal number of clusters is 2. 
...
``` {r fig9, fig.cap="K-means Clustering Result of Transformed Data", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
|lm   0.1328545 0.1450611    0
|knn  0.4811524 0.4973572    0
|rf   0.1342660 0.1365014    0
|cart 0.2010494 0.2050120    0
|gbm  0.1351613 0.1371997    0         |
|---------------------------------------------|
|lm   0.1188522 0.1213054 0.1289166 0.1293980 |
|---------------------------------------------|
|knn  0.4372091 0.4535538 0.4724095 0.4683364 |
|---------------------------------------------|
|rf   0.1295603 0.1295813 0.1339599 0.1327738 |
|---------------------------------------------|
|cart 0.1790975 0.1831987 0.1856227 0.1907960 |
|---------------------------------------------|
|gbm  0.1244813 0.1305179 0.1312647 0.1317250 |
|---------------------------------------------|


\flushleft
From the above table, we can see that linear model has the lowest MAE, Random Forest has the second lowest MAE. Gradient-Boosting model has the lowest RMSE, and Random Forest also has the second lowest RMSE.As a result, we decided to choose these 3 models initially and do the advanced feature engineering.
```{r warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
model_list <- list(lm = lm_cv, rf = rf_cv, gbm = gbm_cv)
...
```
\flushleft
Depending on the above table, after the advanced feature engineering, we saw that random forest model has the best performance, which has the lowest MAE, lowest RMSE and highest R-square. 
\flushleft
So we chose Random Forest model to do the prediction of total auditing fee based on other variables.
```{r fig11, fig.cap="Relationship Between Error and Number of Trees in Random Forest Model", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```{r fig12, fig.cap="Influential Variables on Total Fee ranked by IncMSE", warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
...
```
\flushleft
Then we saw the RMSE of random forest model(rf1_200) is 0.5864.
From the industrial knowledge, the size of company and the brand of audit company are important factors of audit fee. As a result, we chose revenues, earnings, assets and market caps as the characteristics of the company. Additionally, we chose five category factor as the indicator of the brand of the audit company: 1-4 for Big 4 firms and 5 for non-Big 4 firms. Revenues_bc has the most influence on the total fees as we expected, if we delete the revenue variable, the accuracy of model will decrease about 28%. The other 5 variables: the region of the company, the market cap of the company, the asset of the company, the earnings of the company and whether the company is audited by the big 4, also have major influence on the model. 
```{r fig13, fig.cap="Performance Evaluation of Predictive Model",warning=FALSE, echo=FALSE, message=FALSE, alert=FALSE}
