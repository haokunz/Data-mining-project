...
author: "Haokun Zhang, Zhang Lu, Jyun-Yu Cheng"
date: '2023-04-17'
output: pdf_document
---
...
restate2$Current_big4 <- as.factor(as.integer(restate2$Current_big4))
restate2$Disclosure_big4 <- as.factor(as.integer(restate2$Disclosure_big4))
restate2$resated_big4 <- as.factor(as.integer(restate2$resated_big4))
restate2$opinion_big4 <- as.factor(as.integer(restate2$opinion_big4))
...
restate2[c(28:39)]<- sapply(restate2[c(28:39)], 
                                           clean_string2)

# assign failure indicator
restate2$GAAP_Failure_Key<- as.factor(ifelse(restate2$Accounting.Rule..GAAP.FASB..Application.Failures =="", 0,1))
restate2$Financial_Fraud_Key <- as.factor(ifelse(restate2$Financial.Fraud..Irregularities.and.Misrepresentations == "", 0, 1))
restate2$Accounting_Error <- as.factor(ifelse(restate2$Errors...Accounting.and.Clerical.Applications == "", 0,1))
restate2$Other_Issue_key <- as.factor(ifelse(restate2$Other.Significant.Issues == "", 0, 1))

# create ratios of net income and stockholder's equity to market cap
restate2$income_MR_market_ratio = restate2$Cumulative.Change.in.Net.Income/restate2$MR...Market.Cap....
restate2$income_H_market_ratio = restate2$Cumulative.Change.in.Net.Income/restate2$H...Market.Cap....
restate2$equity_MR_market_ratio = restate2$Cumulative.Change.in.Stockholder.Equity/restate2$MR...Market.Cap....
restate2$equity_H_market_ratio = restate2$Cumulative.Change.in.Stockholder.Equity/restate2$H...Market.Cap....
...
# take natural log of audit fee
total$log_fee <- log(total$audit_fees)

# count disclosure frequency
# disclosure_count <- total %<% distinct(Product,Patient_ID) %<% group_by(Product)%>% summarize ("Freq" = n())
```
